...
## 11. 2026-09-22 · 排行榜改造（F1 闭环）

F1（排行榜日榜/周榜空列表）在这一轮彻底解决，并且发现原判断只对了一半。

### 11.1 事实复核：只有日榜是死的

| 档位 | 参数 | 实测（连测 6 轮稳定） | 结论 |
|---|---|---|---|
| 日榜 | `o=mv_t` | `total=0`，恒空 | ❌ 服务端该档无数据 |
| 周榜 | `o=mv_w` | `total=32~45` | ✅ **已恢复**（此前观测为空） |
| 月榜 | `o=mv_m` | `total=3601` | ✅ |

参数本身没写错——参考实现 `JMComic-Crawler-Python/src/jmcomic/jm_config.py`
的 `ORDER_DAY_RANKING='mv_t'` / `ORDER_WEEK_RANKING='mv_w'` / `ORDER_MONTH_RANKING='mv_m'`
与代码一致。之前那句「日榜与周榜都稳定返回 total=0」的注释已经过期。

另外确认了一条**负向结论**：`t=` 时间段参数（`t/w/m/a`）在移动端接口上**完全无效** ——
`/categories/filter` 与 `/search` 上四个取值的返回结果一模一样。
所以「时间段 + 最多观看」拼不出等效的日榜，这条路堵死。

### 11.2 日榜位置 → 「新晋热榜」（本地重排，非服务端档位）

取月榜前 2 页当热度池（160 条），按 `update_at` 倒序取前 40。

实测热度池的日期分布（决定了这个设计可行）：

| 字段 | 近 24h | 近 7 天 | 近 30 天 | >30 天 |
|---|---|---|---|---|
| `update_at`（最近更新） | 29 | 108 | 160 | 0 |
| `adddate`（首次发布日） | 0 | 3 | 13 | 144 |

⚠ **必须用 `update_at`，不能用 `adddate`**：后者对老作品是重传前的原始发布日，
用它筛「近 7 天」只能得到 3 条。为此 `ComicSummary` 新增 `lastUpdatedAt`
（原来的 `update_at` 在 `toSummary` 里被丢掉了）。

取 40 条时覆盖最近约一两天，语义上最接近「日榜」。

### 11.3 档位能力自愈

档位不再写死：`JmRepository.availableRankTabs()` 探测 `mv_w` / `mv_m` 是否有数据，
空档位自动隐藏，结果缓存 6 小时。UI 先按**乐观全集**渲染（避免等探测才出 tab 导致首屏空白），
探测完成后收敛；当前档位被移除则自动切到首个可用档位；全部为空则回退乐观全集。

这与「域名四级自愈」是同一套思路：**能自愈的东西不该写死**。
档位写死过一次就出过空 tab（首屏一片空白），这次把它变成可自愈的。

### 11.4 顺带修掉的日期显示错误（0.1.4）

上线「新晋热榜」后发现卡片日期对不上——一排 2019/2020 年的日期出现在「新晋」标签下。

根因：`toSummary` 里 `updatedAt = if (adddate.isNotBlank()) adddate else formatDate(updateAt)`
**优先级反了**。列表的服务端顺序本来就是按 `update_at` 排的：

```
o=mr    adddate 降序? false   update_at 降序? true
   adddate  : 2026-08-17, 2026-07-27, 2026-07-13, 2026-06-15, 2026-06-08
   update_at: 2026-09-21, 2026-09-21, 2026-09-21, 2026-09-21, 2026-09-21
```

而 `updatedAt` 这个字段的注释写的是「更新时间」、详情页渲染成「更新 …」、
关注流按它排序——全是「更新」语义，却装了「发布」的值。

改为 `displayUpdatedAt(updateAt, adddate)`：优先 `update_at`，缺失才退回 `adddate`。
一处改动同时修正：列表卡片日期、详情页「更新 …」、关注流按更新排序、DD/DA 本地排序。

### 11.5 金丝雀与测试配套

- 日榜从「必须断言失败」改为 **INFO 信息项**（恢复与否都只提示，不计入通过率）——
  为此给 `jm-api-check.mjs` 加了 `notes` 概念。
- 断言只要求周/月两档有数据；新增「月榜池含足够近 7 天有更新的作品」断言
  （新晋热榜的数据前提）；`hasRank` 口径改为只看周/月。
- 新增 `RankTabTest`（12 个用例）：档位映射、`pickNewArrivals` 的排序/回退/空池/上限、
  以及日期优先级与边界（钉死时区避免跨机器飘）。

金丝雀 **32/32 通过**（此前 31/32）；单测 46 → **58** 个用例。

### 11.6 真机验收（v0.1.4）

| 项 | 结果 |
|---|---|
| 默认档位「新晋热榜」 | ✅ `共 40 部`，卡片日期 `2026-09-21`，首屏无空白 |
| 周榜 | ✅ `共 45 部` |
| 月榜 | ✅ `共 3601 部`（含「已读」角标） |
| 三档切换 | ✅ 无空档位、无闪烁 |
| 发布链路 | ✅ raw `update.json` 200 / raw APK 200 / **下载 sha256 == 声明值** |

版本推进：`0.1.3`（榜单改造）→ `0.1.4`（日期修正）。
